...
static void labs_audio_sender_frame(LabsAudioSender *audio_sender, uint8_t *buf, size_t buf_size);
LABS_EXPORT LabsErrorCode labs_audio_sender_init(LabsAudioSender *audio_sender, LabsLog *log, LabsSession *session)
{
    ...
    LabsErrorCode err = labs_mutex_init(&audio_sender->mutex, false);
...
LABS_EXPORT void labs_audio_sender_fini(LabsAudioSender *audio_sender)
{
    ...
    labs_mutex_fini(&audio_sender->mutex);
}

LABS_EXPORT void labs_audio_sender_opus_data(LabsAudioSender *audio_sender, uint8_t *opus_sender, size_t opus_sender_size)
{
    ...
    if(!audio_sender->frameb)
    {
        ...
        if(!audio_sender->frameb)
        {
            LABS_LOGE(audio_sender->log, "Error allocating audio frameb");
            return;
        }
    ...
    if(!audio_sender->framea)
    {
        ...
        if(!audio_sender->framea)
        {
            LABS_LOGE(audio_sender->log, "Error allocating audio framea");
            return;
        }
    ...
    *(labs_unaligned_uint16_t *)(audio_sender->filled_packet_buf + 1) = packet_index;
    *(labs_unaligned_uint16_t *)(audio_sender->filled_packet_buf + 3) = frame_index;
    *(labs_unaligned_uint32_t *)(audio_sender->filled_packet_buf + 5) = units_number;
    audio_sender->filled_packet_buf[9] = codec;
    *(labs_unaligned_uint32_t *)(audio_sender->filled_packet_buf + 10) = gmac;
    *(labs_unaligned_uint32_t *)(audio_sender->filled_packet_buf + 14) = key_pos;
	...
	labs_audio_sender_frame(audio_sender, audio_sender->filled_packet_buf, filled_packet_size);
    return;
}

static void labs_audio_sender_frame(LabsAudioSender *audio_sender, uint8_t *buf, size_t buf_size)
{
	LabsErrorCode err = labs_mutex_lock(&audio_sender->mutex);
	if(err != LABS_ERR_SUCCESS)
	{
		LABS_LOGE(audio_sender->log, "Failed to lock audio sender mutex: %s", labs_error_string(err));
		return;
	}

	err = labs_takion_send_mic_packet(audio_sender->takion, buf, buf_size, audio_sender->ps5);
	if(err != LABS_ERR_SUCCESS)
		LABS_LOGE(audio_sender->log, "Failed to send mic audio packet: %s", labs_error_string(err));

	if(audio_sender->frame_index == UINT16_MAX)
		audio_sender->frame_index = 0;
	else
		audio_sender->frame_index++;
	err = labs_mutex_unlock(&audio_sender->mutex);
	if(err != LABS_ERR_SUCCESS)
		LABS_LOGE(audio_sender->log, "Failed to unlock audio sender mutex: %s", labs_error_string(err));
}
