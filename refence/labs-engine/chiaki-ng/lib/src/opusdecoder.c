...
static void labs_opus_decoder_header(LabsAudioHeader *header, void *user);
static void labs_opus_decoder_frame(uint8_t *buf, size_t buf_size, void *user);
...
{
	sink->user = decoder;
	sink->header_cb = labs_opus_decoder_header;
	sink->frame_cb = labs_opus_decoder_frame;
}

static void labs_opus_decoder_header(LabsAudioHeader *header, void *user)
{
	LabsOpusDecoder *decoder = user;
	...
	if(error != OPUS_OK)
	{
		LABS_LOGE(decoder->log, "LabsOpusDecoder failed to initialize opus decoder: %s", opus_strerror(error));
		decoder->opus_decoder = NULL;
		return;
	}

	LABS_LOGI(decoder->log, "LabsOpusDecoder initialized");

	size_t pcm_buf_size_required = labs_audio_header_frame_buf_size(header);
	...
	if(!decoder->pcm_buf)
	{
		free(pcm_buf_old);
		decoder->pcm_buf = NULL;
		LABS_LOGE(decoder->log, "LabsOpusDecoder failed to alloc pcm buffer");
		opus_decoder_destroy(decoder->opus_decoder);
		decoder->opus_decoder = NULL;
		decoder->pcm_buf_size = 0;
		return;
...
static void labs_opus_decoder_frame(uint8_t *buf, size_t buf_size, void *user)
{
	...
	if(!decoder->opus_decoder)
	{
		LABS_LOGE(decoder->log, "Received audio frame, but opus decoder is not initialized");
		return;
	}
	...
	if(r < 1)
		LABS_LOGE(decoder->log, "Decoding audio frame with opus failed: %s", opus_strerror(r));
