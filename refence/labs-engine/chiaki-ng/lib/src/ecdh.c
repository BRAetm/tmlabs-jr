...
#else
#define CHECK(a) if(!(a)) { labs_ecdh_fini(ecdh); return LABS_ERR_UNKNOWN; }
...
#define GOTO_ERROR(err) do { \
	...
	GOTO_ERROR(mbedtls_md_hmac_starts(&ctx, handshake_key, LABS_HANDSHAKE_KEY_SIZE));
...
error:
	...
	*key_out_size = EC_POINT_point2oct(ecdh->group, point, POINT_CONVERSION_UNCOMPRESSED, key_out, *key_out_size, NULL);
	...
	if(!HMAC(EVP_sha256(), handshake_key, LABS_HANDSHAKE_KEY_SIZE, key_out, *key_out_size, sig_out, (unsigned int *)sig_out_size))
		return LABS_ERR_UNKNOWN;
	return LABS_ERR_SUCCESS;
...
#define GOTO_ERROR(err) do { \
	...
	GOTO_ERROR(mbedtls_mpi_write_binary(&ecdh->ctx.z,
		secret_out, LABS_ECDH_SECRET_SIZE));
...
error:
	...
	int r = ECDH_compute_key(secret_out, LABS_ECDH_SECRET_SIZE, remote_public_key, ecdh->key_local, NULL);

	EC_POINT_free(remote_public_key);

	if(r != LABS_ECDH_SECRET_SIZE)
		return LABS_ERR_UNKNOWN;
