...
public sealed class FramePublisher : IDisposable
{
    ...
    // Rolling per-session frame timestamps, trimmed to a 1-second window.
    // GetFps = count of samples inside the window (since window is 1s, count == fps).
    private const int FpsWindowMs = 1000;
    private readonly object _fpsLock = new();
    private readonly System.Collections.Generic.Dictionary<int, System.Collections.Generic.Queue<long>> _frameTimes = new();

    public void NotifyFrame(int sessionId)
    {
        var now = Environment.TickCount64;
        lock (_fpsLock)
        {
            if (!_frameTimes.TryGetValue(sessionId, out var q))
            {
                q = new System.Collections.Generic.Queue<long>();
                _frameTimes[sessionId] = q;
            }
            q.Enqueue(now);
            while (q.Count > 0 && now - q.Peek() > FpsWindowMs) q.Dequeue();
        }
    }

    /// <summary>Frames per second observed for the given session over the last ~1s. 0 if no frames.</summary>
    public double GetFps(int sessionId)
    {
        lock (_fpsLock)
        {
            if (!_frameTimes.TryGetValue(sessionId, out var q)) return 0;
            var now = Environment.TickCount64;
            while (q.Count > 0 && now - q.Peek() > FpsWindowMs) q.Dequeue();
            return q.Count;
        }
    ...
    {
        if (_disposed) return;
        try { _shm.Publish(sessionId, width, height, stride, LabsFrameShm.FormatBgra, bgra); NotifyFrame(sessionId); }
        catch (Exception ex)
        {
