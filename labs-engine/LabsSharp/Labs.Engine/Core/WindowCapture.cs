...
public sealed class WindowCapture : IDisposable
{
    ...
    // When set, SurfaceToBitmapSource crops the captured frame to this sub-rect.
    // Used by StartForMonitor so the caller gets the window region, not the full display.
    ...
    /// <summary>Starts capture directly from an already-created GraphicsCaptureItem
    /// (e.g. one returned by GraphicsCapturePicker). No crop is applied.</summary>
    public void StartWithItem(GraphicsCaptureItem item)
    {
        Log($"[WindowCapture] StartWithItem() — {item.DisplayName} ({item.Size.Width}x{item.Size.Height})");
        ...
        Log("[WindowCapture] StartWithItem: capture session started.");
    }
    ...
    /// <summary>
    /// Captures the monitor that contains <paramref name="hwnd"/> and crops every frame
    /// to the window's bounding rect. This bypasses window-level content protection that
    /// blocks WGC window capture (e.g. Xbox Remote Play DRM swapchains).
    /// </summary>
    public void StartForMonitor(nint hwnd)
    {
        Log($"[WindowCapture] StartForMonitor() called for HWND 0x{hwnd:X}");

        // Determine which monitor the window lives on and get its origin.
        ...
        // Compute crop rect relative to monitor origin.
        GetWindowRect(hwnd, out RECT wr);
        _cropX = wr.Left   - mi.rcMonitor.Left;
        _cropY = wr.Top    - mi.rcMonitor.Top;
        _cropW = wr.Right  - wr.Left;
        _cropH = wr.Bottom - wr.Top;
        Log($"[WindowCapture] Monitor origin=({mi.rcMonitor.Left},{mi.rcMonitor.Top}) crop=({_cropX},{_cropY},{_cropW}x{_cropH})");
        ...
        _lastSize    = _captureItem.Size;
        Log($"[WindowCapture] Monitor capture item: {_lastSize.Width}x{_lastSize.Height}");
        ...
        _session.IsCursorCaptureEnabled = false;
        _session.StartCapture();
        Log($"[WindowCapture] Monitor capture session started.");
    }
        ...
        try
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    try
                    {
                        // Monitor capture: crop to the target window's rect.
                        bool hasCrop = _cropW > 0 && _cropH > 0;
                        int outX = hasCrop ? Math.Max(0, _cropX) : 0;
                        int outY = hasCrop ? Math.Max(0, _cropY) : 0;
                        int outW = hasCrop ? Math.Min(_cropW, size.Width  - outX) : size.Width;
                        int outH = hasCrop ? Math.Min(_cropH, size.Height - outY) : size.Height;
                        ...
                        unsafe
                        {
                            ...
                            for (int row = 0; row < outH; row++)
                                Buffer.MemoryCopy(
                                    src + (long)(outY + row) * mapped.RowPitch + (long)outX * 4,
