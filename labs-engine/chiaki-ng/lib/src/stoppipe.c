...
LABS_EXPORT LabsErrorCode labs_stop_pipe_select_single(LabsStopPipe *stop_pipe, labs_socket_t fd, bool write, uint64_t timeout_ms)
{
	...
	if(!LABS_SOCKET_IS_INVALID(fd) && FD_ISSET(fd, write ? &wfds : &rfds))
		return LABS_ERR_SUCCESS;
...
LABS_EXPORT LabsErrorCode labs_stop_pipe_connect(LabsStopPipe *stop_pipe, labs_socket_t fd, struct sockaddr *addr, size_t addrlen, uint64_t timeout_ms)
{
	...
	if(LABS_SOCKET_EINPROGRESS)
	{
		LabsErrorCode err = labs_stop_pipe_select_single(stop_pipe, fd, true, timeout_ms);
	...
	else
	{
		...
		if(err == WSAECONNREFUSED)
			return LABS_ERR_CONNECTION_REFUSED;
		else
...
LABS_EXPORT LabsErrorCode labs_stop_pipe_reset(LabsStopPipe *stop_pipe)
{
	...
	return r ? LABS_ERR_SUCCESS : LABS_ERR_UNKNOWN;
#elif defined(__SWITCH__)
	//FIXME
	uint8_t v;
	int r;
	while((r = read(stop_pipe->fd, &v, sizeof(v))) > 0);
	return r < 0 ? LABS_ERR_UNKNOWN : LABS_ERR_SUCCESS;
#else
	uint8_t v;
	int r;
