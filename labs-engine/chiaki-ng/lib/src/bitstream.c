...
static bool header_h264(LabsBitstream *bitstream, uint8_t *data, unsigned size)
{
	struct vl_vlc vlc = {0};
	vl_vlc_init(&vlc, data, size);
	if(!skip_startcode(&vlc))
	{
		LABS_LOGW(bitstream->log, "parse_sps_h264: No startcode found");
		return false;
	}
	...
	if(nal_unit_type != 7)
	{
		LABS_LOGW(bitstream->log, "parse_sps_h264: Unexpected NAL unit type %u", nal_unit_type);
		return false;
	}
	...
	{
		LABS_LOGW(bitstream->log, "parse_sps_h264: Unexpected log2_max_frame_num_minus4 value %u", bitstream->h264.sps.log2_max_frame_num_minus4);
		return false;
	}
...
static bool header_h265(LabsBitstream *bitstream, uint8_t *data, unsigned size)
{
	struct vl_vlc vlc = {0};
	vl_vlc_init(&vlc, data, size);
sps_start:
	if(!skip_startcode(&vlc))
	{
		LABS_LOGW(bitstream->log, "parse_sps_h265: No startcode found");
		return false;
	}
	...
	if(nal_unit_type != 33)
	{
		LABS_LOGW(bitstream->log, "parse_sps_h265: Unexpected NAL unit type %u", nal_unit_type);
		return false;
	}
	...
	{
		LABS_LOGW(bitstream->log, "parse_sps_h265: Unexpected log2_max_pic_order_cnt_lsb_minus4 value %u", bitstream->h265.sps.log2_max_pic_order_cnt_lsb_minus4);
		return false;
	}
...
static bool slice_h264(LabsBitstream *bitstream, uint8_t *data, unsigned size, LabsBitstreamSlice *slice)
{
	struct vl_vlc vlc = {0};
	vl_vlc_init(&vlc, data, size);
	if(!skip_startcode(&vlc))
	{
		LABS_LOGW(bitstream->log, "parse_slice_h264: No startcode found");
		return false;
	}
	...
	{
		LABS_LOGW(bitstream->log, "parse_slice_h264: Unexpected NAL unit type %u", nal_unit_type);
		return false;
	}
	...
	switch(vl_rbsp_ue(&rbsp))
	{
		case 0:
		case 5:
			slice->slice_type = LABS_BITSTREAM_SLICE_P;
			break;
		case 2:
		case 7:
			slice->slice_type = LABS_BITSTREAM_SLICE_I;
			break;
		default:
			slice->slice_type = LABS_BITSTREAM_SLICE_UNKNOWN;
			break;
	}
	...
	if(nal_unit_type == 1)
	{
		...
		if(vl_rbsp_u(&rbsp, 1)) // ref_pic_list_modification_flag_l0
		{
			...
			LABS_LOGW(bitstream->log, "parse_slice_h264: Failed to parse ref_pic_list_modification");
			return false;
		}
	}
...
static bool slice_h265(LabsBitstream *bitstream, uint8_t *data, unsigned size, LabsBitstreamSlice *slice)
{
	struct vl_vlc vlc = {0};
	vl_vlc_init(&vlc, data, size);
	...
	if(!skip_startcode(&vlc))
	{
		LABS_LOGW(bitstream->log, "parse_slice_h265: No startcode found");
		return false;
	}
	...
	{
		LABS_LOGW(bitstream->log, "parse_slice_h265: Unexpected NAL unit type %u", nal_unit_type);
		return false;
	}
	...
	if(nal_unit_type == 1)
	{
		...
		if(!vl_rbsp_u(&rbsp, 1)) // short_term_ref_pic_set_sps_flag
		{
			...
			{
				LABS_LOGW(bitstream->log, "parse_slice_h265: Unexpected num_negative_pics %u", num_negative_pics);
				return false;
			}
		...
		if(slice->reference_frame == 0xff)
			LABS_LOGV(bitstream->log, "parse_slice_h265: No ref frame found");
	}

	return true;
}

static bool slice_set_reference_frame_h265(LabsBitstream *bitstream, uint8_t *data, unsigned size, unsigned reference_frame)
{
	struct vl_vlc vlc = {0};
	vl_vlc_init(&vlc, data, size);
	if(!skip_startcode(&vlc))
	{
		LABS_LOGW(bitstream->log, "slice_set_reference_frame_h265: No startcode found");
		return false;
	}
	...
	if(nal_unit_type != 1)
	{
		LABS_LOGW(bitstream->log, "slice_set_reference_frame_h265: Unexpected NAL unit type %u", nal_unit_type);
		return false;
	}
	...
	{
		LABS_LOGW(bitstream->log, "slice_set_reference_frame_h265: Not P slice");
		return false;
	}
	...
	if(!vl_rbsp_u(&rbsp, 1)) // short_term_ref_pic_set_sps_flag
	{
		...
		if(num_negative_pics > 16)
		{
			LABS_LOGW(bitstream->log, "slice_set_reference_frame_h265: Unexpected num_negative_pics %u", num_negative_pics);
			return false;
		}
		vl_rbsp_ue(&rbsp); // num_positive_pics
		for(unsigned i=0; i<num_negative_pics; i++)
...
void labs_bitstream_init(LabsBitstream *bitstream, LabsLog *log, LabsCodec codec)
{
	bitstream->log = log;
	bitstream->codec = codec;
}

bool labs_bitstream_header(LabsBitstream *bitstream, uint8_t *data, unsigned size)
{
	if(bitstream->codec == LABS_CODEC_H264)
	{
...
bool labs_bitstream_slice(LabsBitstream *bitstream, uint8_t *data, unsigned size, LabsBitstreamSlice *slice)
{
	if(bitstream->codec == LABS_CODEC_H264)
...
bool labs_bitstream_slice_set_reference_frame(LabsBitstream *bitstream, uint8_t *data, unsigned size, unsigned reference_frame)
{
	if(bitstream->codec == LABS_CODEC_H264)
		return false;
	else
