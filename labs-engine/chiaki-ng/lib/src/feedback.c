...
LABS_EXPORT void labs_feedback_state_format_v9(uint8_t *buf, LabsFeedbackState *state)
{
	...
	buf[0xf] = qc >> 0x10;
	buf[0x10] = qc >> 0x18;
	*((labs_unaligned_uint16_t *)(buf + 0x11)) = htons((uint16_t)state->left_x);
	*((labs_unaligned_uint16_t *)(buf + 0x13)) = htons((uint16_t)state->left_y);
	*((labs_unaligned_uint16_t *)(buf + 0x15)) = htons((uint16_t)state->right_x);
	*((labs_unaligned_uint16_t *)(buf + 0x17)) = htons((uint16_t)state->right_y);
}

LABS_EXPORT void labs_feedback_state_format_v12(uint8_t *buf, LabsFeedbackState *state)
{
	labs_feedback_state_format_v9(buf, state);
	buf[0x19] = 0x0;
	buf[0x1a] = 0x0;
	buf[0x1b] = 0x1;
}
...
LABS_EXPORT LabsErrorCode labs_feedback_history_event_set_button(LabsFeedbackHistoryEvent *event, uint64_t button, uint8_t state)
{
	...
	switch(button)
	{
		case LABS_CONTROLLER_BUTTON_CROSS:
			event->buf[1] = 0x88;
			break;
		case LABS_CONTROLLER_BUTTON_MOON:
			event->buf[1] = 0x89;
			break;
		case LABS_CONTROLLER_BUTTON_BOX:
			event->buf[1] = 0x8a;
			break;
		case LABS_CONTROLLER_BUTTON_PYRAMID:
			event->buf[1] = 0x8b;
			break;
		case LABS_CONTROLLER_BUTTON_DPAD_LEFT:
			event->buf[1] = 0x82;
			break;
		case LABS_CONTROLLER_BUTTON_DPAD_RIGHT:
			event->buf[1] = 0x83;
			break;
		case LABS_CONTROLLER_BUTTON_DPAD_UP:
			event->buf[1] = 0x80;
			break;
		case LABS_CONTROLLER_BUTTON_DPAD_DOWN:
			event->buf[1] = 0x81;
			break;
		case LABS_CONTROLLER_BUTTON_L1:
			event->buf[1] = 0x84;
			break;
		case LABS_CONTROLLER_BUTTON_R1:
			event->buf[1] = 0x85;
			break;
		case LABS_CONTROLLER_ANALOG_BUTTON_L2:
			event->buf[1] = 0x86;
			break;
		case LABS_CONTROLLER_ANALOG_BUTTON_R2:
			event->buf[1] = 0x87;
			break;
		case LABS_CONTROLLER_BUTTON_L3:
			event->buf[1] = state ? 0xaf : 0x8f;
			return LABS_ERR_SUCCESS;
		case LABS_CONTROLLER_BUTTON_R3:
			event->buf[1] = state ? 0xb0 : 0x90;
			return LABS_ERR_SUCCESS;
		case LABS_CONTROLLER_BUTTON_OPTIONS:
			event->buf[1] = state ? 0xac : 0x8c;
			return LABS_ERR_SUCCESS;
		case LABS_CONTROLLER_BUTTON_SHARE:
			event->buf[1] = state ? 0xad : 0x8d;
			return LABS_ERR_SUCCESS;
		case LABS_CONTROLLER_BUTTON_TOUCHPAD:
			event->buf[1] = state ? 0xb1 : 0x91;
			return LABS_ERR_SUCCESS;
		case LABS_CONTROLLER_BUTTON_PS:
...
{
	feedback_history_buffer->events = calloc(size, sizeof(LabsFeedbackHistoryEvent));
...
LABS_EXPORT LabsErrorCode labs_feedback_history_buffer_format(LabsFeedbackHistoryBuffer *feedback_history_buffer, uint8_t *buf, size_t *buf_size)
{
	...
	{
		LabsFeedbackHistoryEvent *event = &feedback_history_buffer->events[(feedback_history_buffer->begin + i) % feedback_history_buffer->size];
