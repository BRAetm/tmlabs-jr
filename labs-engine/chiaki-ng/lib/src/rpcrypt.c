...
static void bright_ambassador_ps4_pre10(uint8_t *bright, uint8_t *ambassador, const uint8_t *nonce, const uint8_t *morning)
{
	...
	for(uint8_t i=0; i<LABS_RPCRYPT_KEY_SIZE; i++)
	{
		uint8_t v = nonce[i];
		v -= i;
		v -= 0x27;
...
static LabsErrorCode bright_ambassador(LabsTarget target, uint8_t *bright, uint8_t *ambassador, const uint8_t *nonce, const uint8_t *morning)
{
	...
	const uint8_t *keys_a = labs_target_is_ps5(target) ? keys_a_ps5 : keys_a_ps4;
	const uint8_t *keys_b = labs_target_is_ps5(target) ? keys_b_ps5 : keys_b_ps4;

	const uint8_t *key = &keys_a[(nonce[0] >> 3) * 0x70];
	for(size_t i=0; i<LABS_RPCRYPT_KEY_SIZE; i++)
	{
		uint8_t v = nonce[i];
		if(labs_target_is_ps5(target))
		{
			v -= 0x2d;
			v -= i;
		}
...
LABS_EXPORT void labs_rpcrypt_bright_ambassador(LabsTarget target, uint8_t *bright, uint8_t *ambassador, const uint8_t *nonce, const uint8_t *morning)
{
	switch(target)
	{
		...
		case LABS_TARGET_PS4_9:
			...
			break;
		default: {
			LabsErrorCode err = bright_ambassador(target, bright, ambassador, nonce, morning);
...
LABS_EXPORT LabsErrorCode labs_rpcrypt_aeropause(LabsTarget target, size_t key_1_off, uint8_t *aeropause, const uint8_t *ambassador)
{
	...
	if(target < LABS_TARGET_PS4_10)
		return LABS_ERR_INVALID_DATA;
	const uint8_t *keys_1 = labs_target_is_ps5(target) ? ps5_keys_1 : ps4_keys_1;
	uint8_t wurzelbert = labs_target_is_ps5(target) ? -0x2d : 0x29;
...
LABS_EXPORT LabsErrorCode labs_rpcrypt_aeropause_psn(LabsTarget target, size_t key_1_off, uint8_t *aeropause, const uint8_t *ambassador)
{
	...
	if(target < LABS_TARGET_PS4_10)
		return LABS_ERR_INVALID_DATA;
	const uint8_t *keys_1 = labs_target_is_ps5(target) ? ps5_keys_1 : ps4_keys_1;
	uint8_t wurzelbert = labs_target_is_ps5(target) ? 0x2B : -0x29;
...
LABS_EXPORT void labs_rpcrypt_init_auth(LabsRPCrypt *rpcrypt, LabsTarget target, const uint8_t *nonce, const uint8_t *morning)
{
	...
	labs_rpcrypt_bright_ambassador(target, rpcrypt->bright, rpcrypt->ambassador, nonce, morning);
}

LABS_EXPORT void labs_rpcrypt_init_regist_ps4_pre10(LabsRPCrypt *rpcrypt, const uint8_t *ambassador, uint32_t pin)
{
	rpcrypt->target = LABS_TARGET_PS4_9; // representative, might not be the actual version
	static const uint8_t regist_aes_key[LABS_RPCRYPT_KEY_SIZE] =
...
LABS_EXPORT LabsErrorCode labs_rpcrypt_init_regist(LabsRPCrypt *rpcrypt, LabsTarget target, const uint8_t *ambassador, size_t key_0_off, uint32_t pin)
{
	...
	if(target < LABS_TARGET_PS4_10)
		return LABS_ERR_INVALID_DATA;
	const uint8_t *keys_0 = labs_target_is_ps5(target) ? ps5_keys_0 : ps4_keys_0;
...
LABS_EXPORT LabsErrorCode labs_rpcrypt_init_regist_psn(LabsRPCrypt *rpcrypt, LabsTarget target, const uint8_t *ambassador, size_t key_0_off, uint8_t *custom_data1, uint8_t *data1, uint8_t *data2)
{
	...
	LabsRPCrypt customDataCrypt;
	uint8_t encrypted_custom_data1[LABS_RPCRYPT_KEY_SIZE];
	...
	LabsErrorCode err = labs_rpcrypt_encrypt(&customDataCrypt, 0, custom_data1, encrypted_custom_data1, LABS_RPCRYPT_KEY_SIZE);
...
static const uint8_t *rpcrypt_hmac_key(LabsRPCrypt *rpcrypt)
{
...
LABS_EXPORT LabsErrorCode labs_rpcrypt_generate_iv(LabsRPCrypt *rpcrypt, uint8_t *iv, uint64_t counter)
{
	...
	uint8_t buf[LABS_RPCRYPT_KEY_SIZE + 8];
	memcpy(buf, rpcrypt->ambassador, LABS_RPCRYPT_KEY_SIZE);
	buf[LABS_RPCRYPT_KEY_SIZE + 0] = (uint8_t)((counter >> 0x38) & 0xff);
	buf[LABS_RPCRYPT_KEY_SIZE + 1] = (uint8_t)((counter >> 0x30) & 0xff);
	buf[LABS_RPCRYPT_KEY_SIZE + 2] = (uint8_t)((counter >> 0x28) & 0xff);
	buf[LABS_RPCRYPT_KEY_SIZE + 3] = (uint8_t)((counter >> 0x20) & 0xff);
	buf[LABS_RPCRYPT_KEY_SIZE + 4] = (uint8_t)((counter >> 0x18) & 0xff);
	buf[LABS_RPCRYPT_KEY_SIZE + 5] = (uint8_t)((counter >> 0x10) & 0xff);
	buf[LABS_RPCRYPT_KEY_SIZE + 6] = (uint8_t)((counter >> 0x08) & 0xff);
	buf[LABS_RPCRYPT_KEY_SIZE + 7] = (uint8_t)((counter >> 0x00) & 0xff);

	uint8_t hmac[LABS_RPCRYPT_KEY_SIZE];
...
#undef GOTO_ERROR
	memcpy(iv, hmac, LABS_RPCRYPT_KEY_SIZE);
	mbedtls_md_free(&ctx);
	return LABS_ERR_SUCCESS;
error:
	mbedtls_md_free(&ctx);
...
static LabsErrorCode labs_rpcrypt_crypt(LabsRPCrypt *rpcrypt, uint64_t counter, const uint8_t *in, uint8_t *out, size_t sz, bool encrypt)
{
...
#define GOTO_ERROR(err) do { \
	...
	uint8_t iv[LABS_RPCRYPT_KEY_SIZE];
	LabsErrorCode err = labs_rpcrypt_generate_iv(rpcrypt, iv, counter);
	if(err != LABS_ERR_SUCCESS)
		return err;
...
LABS_EXPORT LabsErrorCode labs_rpcrypt_generate_iv(LabsRPCrypt *rpcrypt, uint8_t *iv, uint64_t counter)
{
	...
	uint8_t hmac[32];
	unsigned int hmac_len = 0;
	if(!HMAC(EVP_sha256(), hmac_key, LABS_RPCRYPT_KEY_SIZE, buf, sizeof(buf), hmac, &hmac_len))
		return LABS_ERR_UNKNOWN;

	if(hmac_len < LABS_RPCRYPT_KEY_SIZE)
		return LABS_ERR_UNKNOWN;
...
{
	return labs_rpcrypt_crypt(rpcrypt, counter, in, out, sz, true);
}
...
{
	return labs_rpcrypt_crypt(rpcrypt, counter, in, out, sz, false);
}
