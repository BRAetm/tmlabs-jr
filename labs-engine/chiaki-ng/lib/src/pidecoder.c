...
{
	memset(decoder, 0, sizeof(LabsPiDecoder));

	bcm_host_init();

	if(!(decoder->client = ilclient_init()))
	{
		LABS_LOGE(decoder->log, "ilclient_init failed");
		labs_pi_decoder_fini(decoder);
		return LABS_ERR_UNKNOWN;
	}

	if(OMX_Init() != OMX_ErrorNone)
	{
		LABS_LOGE(decoder->log, "OMX_Init failed");
		labs_pi_decoder_fini(decoder);
		return LABS_ERR_UNKNOWN;
	}
	...
	{
		LABS_LOGE(decoder->log, "ilclient_create_component failed for video_decode");
		labs_pi_decoder_fini(decoder);
		return LABS_ERR_UNKNOWN;
	}
	...
	{
		LABS_LOGE(decoder->log, "ilclient_create_component failed for video_render");
		labs_pi_decoder_fini(decoder);
		return LABS_ERR_UNKNOWN;
	}
	...
	{
		LABS_LOGE(decoder->log, "OMX_SetParameter failed for video parameters");
		labs_pi_decoder_fini(decoder);
		return LABS_ERR_UNKNOWN;
	}
	...
	{
		LABS_LOGE(decoder->log, "OMX_SetParameter failed for render parameters");
		labs_pi_decoder_fini(decoder);
		return LABS_ERR_UNKNOWN;
	}
	...
	{
		LABS_LOGE(decoder->log, "OMX_SetParameter failed for rotation");
		labs_pi_decoder_fini(decoder);
		return LABS_ERR_UNKNOWN;
	}
	...
	{
		LABS_LOGE(decoder->log, "ilclient_enable_port_buffers failed");
		labs_pi_decoder_fini(decoder);
		return LABS_ERR_UNKNOWN;
	}
	...
	LABS_LOGI(decoder->log, "Raspberry Pi Decoder initialized");
...
static bool push_buffer(LabsPiDecoder *decoder, uint8_t *buf, size_t buf_size)
{
	while(buf_size)
	{
		...
		if(!omx_buf)
		{
			LABS_LOGE(decoder->log, "ilclient_get_input_buffer failed");
			return false;
		}
		...
		{
			LABS_LOGW(decoder->log, "OMX Buffer too small, fragmenting to multiple buffers.");
				...
				|| (omx_buf->nFilledLen == 0 && ilclient_wait_for_event(decoder->video_decode, OMX_EventPortSettingsChanged, 131, 0, 0, 1, ILCLIENT_EVENT_ERROR | ILCLIENT_PARAMETER_CHANGED, 10000) == 0)))
		{
			...
			{
				LABS_LOGE(decoder->log, "ilclient_setup_tunnel failed");
				return false;
			}
		...
		{
			LABS_LOGE(decoder->log, "OMX_EmptyThisBuffer failed");
			return false;
		}
	}
	return true;
...
LABS_EXPORT void labs_pi_decoder_set_params(LabsPiDecoder *decoder, int x, int y, int w, int h, bool visible)
{
	...
	if(OMX_SetParameter(ILC_GET_HANDLE(decoder->video_render), OMX_IndexConfigDisplayRegion, &configDisplay) != OMX_ErrorNone)
		LABS_LOGE(decoder->log, "OMX_SetParameter failed for display params");
}
