...
static bool stun_get_external_address_from_server(LabsLog *log, StunServer *server, char *address, uint16_t *port, labs_socket_t *sock, bool ipv4);
 ...
 */
static bool stun_get_external_address(LabsLog *log, char *address, uint16_t *port, StunServer *passed_servers, size_t num_passed_servers, labs_socket_t *sock, bool ipv4)
{
       ...
       for (int i = 0; i < num_passed_servers; i++)
        {
            ...
            if (stun_get_external_address_from_server(log, &passed_servers[i], address, port, sock, ipv4))
                return true;
            LABS_LOGW(log, "Failed to get external address from %s:%d, retrying with another STUN server...", passed_servers[i].host, passed_servers[i].port);
    ...
    if(ipv4 || !ipv6_tried)
    {
        ...
        for (int i = 0; i < num_servers; i++) {
            ...
            LABS_LOGW(log, "Failed to get external address from %s:%d, retrying with another STUN server...", STUN_SERVERS[i].host, STUN_SERVERS[i].port);
            // Only try 1 IPV6 server
            if(!ipv4)
                break;
        }
    }
    LABS_LOGE(log, "Failed to get external address from any STUN server.");
    return false;
}
 ...
 * @return true if successful, false otherwise
 */
LABS_EXPORT bool stun_port_allocation_test(LabsLog *log, char *address, uint16_t *port, int32_t *allocation_increment, bool *random_allocation, StunServer *passed_servers, size_t num_passed_servers, labs_socket_t *sock)
{
        ...
        for (int i = 0; i < num_passed_servers; i++)
        {
            if(port1 == 0)
            {
                ...
                if (!stun_get_external_address_from_server(log, &passed_servers[i], addr1, &port1, sock, true))
                    ...
                    LABS_LOGV(log, "Got response from STUN server %s:%d", passed_servers[i].host, passed_servers[i].port);
            }
            else if(port2 == 0)
            {
                if(LABS_SOCKET_IS_INVALID(*sock))
        ...
        for (int i = 0; i < num_servers; i++)
        {
            if(port1 == 0)
            {
                ...
                if (!stun_get_external_address_from_server(log, &STUN_SERVERS[i], addr1, &port1, sock, true))
                    ...
                    LABS_LOGV(log, "Got response from STUN server %s:%d", STUN_SERVERS[i].host, STUN_SERVERS[i].port);
            }
            else if(port2 == 0)
            {
                if(LABS_SOCKET_IS_INVALID(*sock))
    ...
    {
        LABS_LOGW(log, "Only 1 STUN server responded for packet allocation calculation.");
        LABS_LOGW(log, "Couldn't determine packet allocation because not enough STUN servers responded.");
    ...
    else if(port3 == 0)
    {
        ...
        LABS_LOGW(log, "Only 2 STUN servers responded for packet allocation calculation.");
        // address changed between requests use most recent one
        if(strcmp(addr1, addr2) != 0)
        {
            LABS_LOGW(log, "Got different addresses between 2 responses, using 2nd one...");
            LABS_LOGW(log, "Couldn't determine packet allocation because not enough STUN servers responded with the same address defaulting to 0.");
            *allocation_increment = 0;
        }
        else
        {
    ...
    {
        LABS_LOGW(log, "Only 3 STUN servers responded for packet allocation calculation.");
        memcpy(address, addr3, sizeof(addr3));
        if(strcmp(addr1, addr2) != 0)
        {
            if(strcmp(addr1, addr3) == 0)
            {
                LABS_LOGW(log, "Calculating packet allocation based on 2 responses with the same address");
            ...
            {
                LABS_LOGW(log, "Got 3 different addresses between 3 responses, using 3rd one...");
                LABS_LOGW(log, "Couldn't determine packet allocation because not enough STUN servers responded with the same address.");
                *allocation_increment = 0;
            }
        }
        else
        {
            ...
            {
                LABS_LOGW(log, "Calculating packet allocation based on 3 responses with the same address");
                *allocation_increment = port2 - port1;
                ...
                if(allocation_increment1 != (*allocation_increment))
                {
                    *random_allocation = true;
                    ...
                    LABS_LOGW(log, "Got different allocation increment calculations from different ports.\nIncrement0: %d, Increment1: %d", *allocation_increment, allocation_increment1);
                }
            }
        }
        ...
        *port = port4;
        if(strcmp(addr1, addr2) != 0)
        {
            ...
            else
            {
                ...
                {
                    LABS_LOGW(log, "Got 4 different addresses between 4 responses, using 4th one...");
                    LABS_LOGW(log, "Couldn't determine packet allocation because not enough STUN servers responded with the same address.");
                    *allocation_increment = 0;
                }
            }
        ...
        {
            LABS_LOGI(log, "Calculating packet allocation based on 4 responses with the same address");
            ...
            if((increment0 == increment1) && (increment1 == increment2))
            {
                *allocation_increment = increment0;
                LABS_LOGV(log, "Got 3 idential allocation increment calculations out of 3,\nIncrement %d", increment0);
            }
            else if (increment0 == increment1 || increment0 == increment2)
            {
                LABS_LOGV(log, "Got 2 idential allocation increment calculations out of 3 from different ports.\nIncrement 0: %d, Increment 1: %d, Increment 2: %d", increment0, increment1, increment2);
                *allocation_increment = increment0;
            }
...
static bool stun_get_external_address_from_server(LabsLog *log, StunServer *server, char *address, uint16_t *port, labs_socket_t *sock, bool ipv4)
{
    ...
    if (getaddrinfo(server->host, server_service, &hints, &resolved) != 0) {
        LABS_LOGE(log, "remote/stun.h: Failed to resolve STUN server '%s', error was " LABS_SOCKET_ERROR_FMT, server->host, LABS_SOCKET_ERROR_VALUE);
        return false;
    }
    ...
    *(int*)(&binding_req[4]) = htonl(STUN_MAGIC_COOKIE);
    labs_random_bytes_crypt(&binding_req[8], STUN_TRANSACTION_ID_LENGTH);

    LABS_SSIZET_TYPE sent = sendto(*sock, (LABS_SOCKET_BUF_TYPE)binding_req, sizeof(binding_req), 0, server_addr, server_addr_len);
    if (sent != sizeof(binding_req)) {
        LABS_LOGE(log, "remote/stun.h: Failed to send STUN request, error was " LABS_SOCKET_ERROR_FMT, LABS_SOCKET_ERROR_VALUE);
        if (!LABS_SOCKET_IS_INVALID(*sock))
        {
    ...
    timeout.tv_usec = 0;
#endif
    if (setsockopt(*sock, SOL_SOCKET, SO_RCVTIMEO, (const LABS_SOCKET_BUF_TYPE)&timeout, sizeof(timeout)) < 0) {
        LABS_LOGE(log, "remote/stun.h: Failed to set socket timeout, error was " LABS_SOCKET_ERROR_FMT, LABS_SOCKET_ERROR_VALUE);
        if (!LABS_SOCKET_IS_INVALID(*sock))
        {
            LABS_SOCKET_CLOSE(*sock);
            *sock = LABS_INVALID_SOCKET;
    ...
    LABS_SSIZET_TYPE received = recvfrom(*sock, (LABS_SOCKET_BUF_TYPE)binding_resp, sizeof(binding_resp), 0, NULL, NULL);
    if (received < 0) {
        LABS_LOGE(log, "remote/stun.h: Failed to receive STUN response, error was " LABS_SOCKET_ERROR_FMT, LABS_SOCKET_ERROR_VALUE);
        return false;
    }

    if(received < STUN_HEADER_SIZE)
    {
        LABS_LOGE(log, "remote/stun.h: Received message is too small");
        return false;
    }


    if (*(uint16_t*)(&binding_resp[0]) != htons(STUN_MSG_TYPE_BINDING_RESPONSE)) {
        LABS_LOGE(log, "remote/stun.h: Received STUN response with invalid message type");
        return false;
    }
    ...
    if (received != expected_size) {
        LABS_LOGE(log, "remote/stun.h: Received STUN response with invalid length: %zd received, %zu expected", received, expected_size);
        return false;
    }

    if (*(int*)(&binding_resp[4]) != htonl(STUN_MAGIC_COOKIE)) {
        LABS_LOGE(log, "remote/stun.h: Received STUN response with invalid magic cookie");
        return false;
    }

    if (memcmp(&binding_resp[8], &binding_req[8], STUN_TRANSACTION_ID_LENGTH) != 0) {
        LABS_LOGE(log, "remote/stun.h: Received STUN response with invalid transaction ID");
        return false;
    }
    ...
    while (response_pos < (received - 3))
    {
        ...
        {
            LABS_LOGE(log, "remote/stun.h: Received STUN response with invalid data");
            return false;
        }
        ...
        if (family == STUN_MAPPED_ADDR_FAMILY_IPV4) {
            if (attr_length != 8) {
                 LABS_LOGE(log, "remote/stun.h: Received STUN_MAPPED_ADDR_FAMILY_IPV4 with invalid data!");
                 LABS_LOGE(log, "remote/stun.h: Expected atribute length: 8. Received attribute length %u", attr_length);
                 return false;
            }
            if (xored) {
            ...
            if (attr_length != 20) {
                 LABS_LOGE(log, "remote/stun.h: Received STUN_MAPPED_ADDR_FAMILY_IPV6 with invalid data!");
                 LABS_LOGE(log, "remote/stun.h: Expected atribute length: 20. Received attribute length %"PRId16, attr_length);
                 return false;
            }
            if (xored) {
                uint16_t xored_port = *(uint16_t*)(&binding_resp[response_pos + 6]) ^ (uint16_t)(htonl(STUN_MAGIC_COOKIE));
            ...
            LABS_LOGE(log, "remote/stun.h: Received STUN response with invalid address family: %d", family);
            return false;
        }
