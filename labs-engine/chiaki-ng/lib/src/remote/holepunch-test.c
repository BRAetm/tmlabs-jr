...
{
    printf("liblabs UDP Holepunching Test\n");
    ...
    LabsLog log;
	labs_log_init(&log, LABS_LOG_ALL, labs_log_cb_print, NULL);

    // List available devices
    LabsHolepunchDeviceInfo *device_info_ps5;
    size_t num_devices_ps5;

    LabsErrorCode err = labs_holepunch_list_devices(oauth_token, LABS_HOLEPUNCH_CONSOLE_TYPE_PS5, &device_info_ps5, &num_devices_ps5, &log);
    ...
    if (num_devices_ps5 > 0)
    {
        ...
        console_type = LABS_HOLEPUNCH_CONSOLE_TYPE_PS5;
    ...
    LabsHolepunchSession session = labs_holepunch_session_init(oauth_token, &log);
    if (!session)
    {
    ...
    err = labs_holepunch_session_create(session);
    if (err != LABS_ERR_SUCCESS)
    {
    ...
    err = labs_holepunch_session_start(session, device_uid, console_type);
    if (err != LABS_ERR_SUCCESS)
    {
        fprintf(stderr, "!! Failed to start session\n");
        labs_holepunch_session_fini(session);
        return -1;
    }
    printf(">> Started session\n");

    err = labs_holepunch_session_punch_hole(session, LABS_HOLEPUNCH_PORT_TYPE_CTRL);
    if (err != LABS_ERR_SUCCESS)
    {
    ...
    printf(">> Punched hole for control connection!\n");
    {
        ...
        if (!labs_holepunch_session_get_stun_allocation(session, &allocation_increment, &random_allocation))
        {
...
cleanup:
    labs_holepunch_free_device_list(&device_info_ps5);
    labs_holepunch_session_fini(session);
