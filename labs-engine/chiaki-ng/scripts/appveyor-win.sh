...
cmake \
	...
	-DLABS_ENABLE_TESTS=ON \
	-DLABS_ENABLE_CLI=OFF \
	-DLABS_GUI_ENABLE_SDL_GAMECONTROLLER=ON \
	..
...
mkdir Labs && cp build/gui/labs.exe Labs
mkdir Labs-PDB && cp build/gui/labs.pdb Labs-PDB

"$QT_PATH/bin/windeployqt.exe" Labs/labs.exe
cp -v $COPY_DLLS Labs
